$PBExportHeader$uo_seleccion_areas.sru
$PBExportComments$Objeto público para selección de Planta, Todas o Consolidada
forward
global type uo_seleccion_areas from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_areas
end type
type cbx_todos from checkbox within uo_seleccion_areas
end type
type dw_seleccion from datawindow within uo_seleccion_areas
end type
end forward
...
global uo_seleccion_areas uo_seleccion_areas
...
public subroutine filtra (integer ai_zona, string as_empresa)
public function boolean inicia (string as_empresa, integer ai_zona, integer ai_codigo)
...
public subroutine filtra (integer ai_zona, string as_empresa);Empresa = as_Empresa
Zona	=	ai_Zona
...
If idwc_Seleccion.Retrieve(Zona, Empresa) = 0 Then
...
public function boolean inicia (string as_empresa, integer ai_zona, integer ai_codigo);Integer	li_Nula
Boolean	lb_Retorno = False

SetNull(li_Nula)

If iuo_Codigo.Of_Existe(ai_zona, as_empresa, ai_Codigo,  False, SQLCA) Then
...
on uo_seleccion_areas.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name				=	'dw_mues_casinoareas'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn		=	'caar_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn			=	'caar_codigo'

dw_Seleccion.GetChild("codigo", idwc_Seleccion)

idwc_Seleccion.SetTransObject(sqlca)
If	idwc_Seleccion.Retrieve(-1, '*') = 0 Then
	MessageBox("Atención", "No existen Areas en Tabla Respectiva.", Information!, Ok!)
	SetNull(Codigo)
	SetNull(Nombre)
Else
	idwc_Seleccion.SetSort("caar_nombre A")
	idwc_Seleccion.Sort()
...
If iuo_Codigo.of_Existe(Zona, Empresa, Integer(data),True, SQLCA) Then
	Codigo			=	iuo_Codigo.Codigo
	Nombre			=	iuo_Codigo.Nombre
Else
