$PBExportHeader$uo_seleccion_centrocosto.sru
$PBExportComments$Objeto público para selección de Centro de Costos, Todas o Consolidada
forward
global type uo_seleccion_centrocosto from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_centrocosto
end type
type cbx_todos from checkbox within uo_seleccion_centrocosto
end type
type dw_seleccion from datawindow within uo_seleccion_centrocosto
end type
end forward
...
global uo_seleccion_centrocosto uo_seleccion_centrocosto

type variables
DataWindowChild	idwc_Seleccion

uo_Centrocosto iuo_Codigo
...
public function boolean inicia (string as_empresa, integer ai_codigo)
public subroutine filtra (string as_empresa)
...
public function boolean inicia (string as_empresa, integer ai_codigo);Integer	li_Nula
Boolean	lb_Retorno = False

SetNull(li_Nula)

If iuo_Codigo.Of_Existe(as_empresa, ai_Codigo,  False, SQLCA) Then
...
public subroutine filtra (string as_empresa);Empresa = as_Empresa
...
on uo_seleccion_centrocosto.create
...
on uo_seleccion_centrocosto.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name				=	'dw_mues_centrocostos_8'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn		=	'ccos_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn			=	'ccos_codigo'

dw_Seleccion.GetChild("codigo", idwc_Seleccion)

idwc_Seleccion.SetTransObject(sqlca)
If	idwc_Seleccion.Retrieve('*') = 0 Then
	MessageBox("Atención", "No existen Centros de Costo en Tabla Respectiva.", Information!, Ok!)
	SetNull(Codigo)
	SetNull(Nombre)
Else
	idwc_Seleccion.SetSort("ccos_nombre A")
	idwc_Seleccion.Sort()
	...
	iuo_Codigo	=	Create uo_Centrocosto
...
If iuo_Codigo.of_Existe(Empresa, Integer(data),True, SQLCA) Then
	Codigo			=	iuo_Codigo.Codigo
	Nombre			=	iuo_Codigo.Nombre
Else
