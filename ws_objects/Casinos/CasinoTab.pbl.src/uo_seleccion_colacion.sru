$PBExportHeader$uo_seleccion_colacion.sru
$PBExportComments$Objeto público para selección de Colaciones, Todas o Consolidada
forward
global type uo_seleccion_colacion from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_colacion
end type
type cbx_todos from checkbox within uo_seleccion_colacion
end type
type dw_seleccion from datawindow within uo_seleccion_colacion
end type
end forward
...
global uo_seleccion_colacion uo_seleccion_colacion

type variables
DataWindowChild	idwc_Seleccion

uo_Casino_Colacion iuo_Codigo

Long		Codigo, Zona, Tipo
String 	Nombre
end variables
...
public function boolean inicia (integer ai_zona, integer ai_tipo, integer ai_codigo)
public subroutine filtra (integer ai_zona, integer ai_tipo)
...
public function boolean inicia (integer ai_zona, integer ai_tipo, integer ai_codigo);Integer	li_Nula
Boolean	lb_Retorno = False

SetNull(li_Nula)

If iuo_Codigo.Of_Existe(ai_Zona, ai_Tipo, ai_Codigo,  False, SQLCA) Then
...
public subroutine filtra (integer ai_zona, integer ai_tipo);Zona	=	ai_Zona	
Tipo	=	ai_Tipo
...
If idwc_Seleccion.Retrieve(Zona, Tipo) = 0 Then
	MessageBox("Atención", "No existen Centros de Costo para Empresa seleccionada.")
...
on uo_seleccion_colacion.create
...
on uo_seleccion_colacion.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name				=	'dw_mues_colaciones'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn		=	'caco_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn			=	'caco_codigo'

dw_Seleccion.GetChild("codigo", idwc_Seleccion)

idwc_Seleccion.SetTransObject(sqlca)
If	idwc_Seleccion.Retrieve(-1, -1) = 0 Then
	MessageBox("Atención", "No existen Colaciones en Tabla Respectiva.", Information!, Ok!)
	SetNull(Codigo)
	SetNull(Nombre)
Else
	idwc_Seleccion.SetSort("caco_nombre A")
	idwc_Seleccion.Sort()
	...
	iuo_Codigo	=	Create uo_Casino_Colacion
...
If iuo_Codigo.of_Existe(Zona, Tipo, Integer(data),True, SQLCA) Then
	Codigo			=	iuo_Codigo.Codigo
	Nombre			=	iuo_Codigo.Nombre
Else
