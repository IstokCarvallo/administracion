$PBExportHeader$uo_seleccion_comuna.sru
$PBExportComments$Objeto público para selección de comunas, Todas o Consolidada
forward
global type uo_seleccion_comuna from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_comuna
end type
type cbx_todos from checkbox within uo_seleccion_comuna
end type
type dw_seleccion from datawindow within uo_seleccion_comuna
end type
end forward
...
global uo_seleccion_comuna uo_seleccion_comuna
...
public subroutine filtra (integer ai_pais, integer ai_region)
...
public subroutine filtra (integer ai_pais, integer ai_region);Pais		=	ai_Pais
Region	= ai_Region
...
IF idwc_Seleccion.Retrieve(Pais, Region) = 0 THEN
	MessageBox("Atención", "No existen Comunas para Region seleccionada.")
...
If iuo_Codigo.Existe(Pais, Region, Integer(ai_Codigo), False, sqlca) Then
...
on uo_seleccion_comuna.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name			=	'dw_mues_comunas'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn	=	'comu_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn		=	'comu_codigo'
...
IF iuo_Codigo.Existe(Pais, Region, Integer(Data), True, sqlca) THEN
	Codigo	=	iuo_Codigo.Codigo
	Nombre	=	iuo_Codigo.Nombre
ELSE
