$PBExportHeader$uo_seleccion_region.sru
$PBExportComments$Objeto público para selección de regiones, Todas o Consolidada
forward
global type uo_seleccion_region from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_region
end type
type cbx_todos from checkbox within uo_seleccion_region
end type
type dw_seleccion from datawindow within uo_seleccion_region
end type
end forward
...
global uo_seleccion_region uo_seleccion_region

type variables
DataWindowChild	idwc_Seleccion

uo_Region	iuo_Codigo
...
public subroutine filtra (integer ai_pais)
...
public subroutine filtra (integer ai_pais);Pais	=	ai_Pais
 
dw_Seleccion.GetChild("codigo", idwc_Seleccion)

idwc_Seleccion.SetTransObject(sqlca)

IF idwc_Seleccion.Retrieve(Pais) = 0 THEN
...
If iuo_Codigo.Existe(Pais, Integer(ai_Codigo), False, sqlca) Then
...
on uo_seleccion_region.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name			=	'dw_mues_region'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn	=	'regi_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn		=	'regi_codigo'
...
IF iuo_Codigo.Existe(Pais, Integer(data), True, sqlca) THEN
	Codigo	=	iuo_Codigo.Codigo
	Nombre	=	iuo_Codigo.Nombre
ELSE
