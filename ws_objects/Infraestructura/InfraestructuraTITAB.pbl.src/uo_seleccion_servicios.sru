$PBExportHeader$uo_seleccion_servicios.sru
$PBExportComments$Objeto público para selección de Servicios, Todas o Consolidada
forward
global type uo_seleccion_servicios from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_servicios
end type
type cbx_todos from checkbox within uo_seleccion_servicios
end type
type dw_seleccion from datawindow within uo_seleccion_servicios
end type
end forward
...
global uo_seleccion_servicios uo_seleccion_servicios

type variables
DataWindowChild	idwc_Seleccion

uo_Servicio iuo_Codigo
...
on uo_seleccion_servicios.create
...
on uo_seleccion_servicios.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name			=	'dw_mues_servicio'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn	=	'serv_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn		=	'serv_codigo'
...
IF	idwc_Seleccion.Retrieve() = 0 THEN
	MessageBox("Atención", "No existen Servicios en tabla respectiva")
...
If iuo_Codigo.Existe(Integer(Data), True, Sqlca) Then
	Codigo			=	iuo_Codigo.Codigo
	Nombre			=	iuo_Codigo.Nombre
	Moneda			=	iuo_Codigo.Moneda
	Valor				=	iuo_Codigo.Valor
	Tiempo			=	iuo_Codigo.Tiempo
Else
