$PBExportHeader$uo_seleccion_tipomovto.sru
$PBExportComments$Objeto público para selección de Tipo de Movimiento Equipos, Todas o Consolidada
forward
global type uo_seleccion_tipomovto from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_tipomovto
end type
type cbx_todos from checkbox within uo_seleccion_tipomovto
end type
type dw_seleccion from datawindow within uo_seleccion_tipomovto
end type
end forward
...
global uo_seleccion_tipomovto uo_seleccion_tipomovto

type variables
DataWindowChild	idwc_Seleccion

uo_TipoMovto iuo_Codigo
...
If iuo_Codigo.Existe(Integer(ai_Codigo), False, sqlca) Then
	...
	TipoTraslado	=	iuo_Codigo.TipoTraslado
	TipoDespacho	=	iuo_Codigo.TipoDespacho
	SolicitaDestino	=	iuo_Codigo.SolicitaDestino
	Glosa				=	iuo_Codigo.Glosa
	Estado			=	iuo_Codigo.Estado
	EmiteGuia		=	iuo_Codigo.EmiteGuia
	CreaEquipo		=	iuo_Codigo.CreaEquipo
...
on uo_seleccion_tipomovto.create
...
on uo_seleccion_tipomovto.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name			=	'dw_mues_tipomovto'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn	=	'tpmv_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn		=	'tpmv_codigo'
...
IF	idwc_Seleccion.Retrieve() = 0 THEN
	MessageBox("Atención", "No existen Tipo Movimientos Equipos en tabla respectiva")
...
ELSE
	...
	Codigo			=	-1
	Nombre			=	'Todas'
	iuo_Codigo		=	Create uo_TipoMovto
